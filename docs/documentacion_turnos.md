...
- **Habilitación CMA/PSA (v6.55):** un **alumno** (`rol==='alumno'`, no aplica a piloto) necesita `vencimiento_cma` (psicofísico) y `vencimiento_psa` (credencial PSA) vigentes para poder pedir turno. Función `chequearHabilitacionAlumno(alumno)`: compara cada fecha contra hoy (`fmtDate`); si está vencida o falta, entra en la lista de bloqueo salvo por la constante `CMA_PSA_VACIO_BLOQUEA`. **`CMA_PSA_VACIO_BLOQUEA=false`** (hoy): un campo **vacío** (todavía sin cargar para ese alumno) **no bloquea** — es un fail-safe temporal mientras administración termina de cargar los vencimientos de todos los legajos existentes; si se pusiera en `true` de entrada, se le cortaría el turno a golpe a todos los alumnos sin vencimiento cargado. Una vez completada la carga, cambiar la constante a `true` (una sola línea) para que un campo vacío también bloquee. Tres capas de enforcement: (1) banner persistente al loguear/restaurar sesión (`renderBannerVencimientoAlumno`), (2) bloqueo completo de la tab "Pedir Turno" con detalle del vencimiento, (3) re-chequeo defensivo dentro de `confirmarTurno` con un fetch fresco de Firebase (cubre el caso de que la sesión ya esté abierta y el vencimiento se cargue mientras tanto). Editable por instructor/admin vía modal EDITAR USUARIO (`#edit-al-cma`/`#edit-al-psa`); solo lectura en Mi Perfil del propio alumno; badge "⚠ VENCIDO" en la lista de usuarios de admin/instructor. **`+ CARGAR TURNO` (staff) bypassea esta restricción a propósito** — mismo patrón que ya existe para la cuota semanal (punto anterior): una carga manual de staff no está sujeta a las reglas pensadas para el flujo de autogestión del alumno.
