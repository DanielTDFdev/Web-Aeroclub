...
- **Seguridad / Auth:** Fases 1 y 2 hechas (modo sombra). Se dejó **decantar la migración** (los usuarios migran al entrar). Próximo paso de desarrollo: Fase 3 (flujos de contraseña a Auth), que destraba endurecer reglas (Fase 5) y sacar el texto plano (Fase 6). Antes de la Fase 6 hay que migrar las 3 cuentas de instructor con clave corta (`fherlein`, `scarrizo`, `sdelarminat`) reseteándolas a 6+.
