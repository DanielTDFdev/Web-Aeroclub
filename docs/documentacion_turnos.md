...
**Una sola cuenta de EmailJS en uso (más una vieja, en desuso, ver abajo).**

- **Cuenta A** — login del propio Daniel (`dcamargo70`), pero con la public key **`0IoRm88tZQepkHAdK`**. Tiene conectado el servicio **`service_8yqlptz`** ("Admin Aeroclub"), cuyo **Gmail Connect real es `aeroclubgra@gmail.com`** (no `dcamargo70@gmail.com` — el *Service ID* mostrado en el dashboard de EmailJS es un campo de texto libre/renombrable, no refleja necesariamente qué Gmail está conectado; hay que mirar el campo "Gmail Connect" puntual de cada servicio para saber con certeza). Templates: cancelación (`mailCancel`), reset (`mailReset`), registro + bienvenida (`mailNuevoRegistro`), confirmación de turno (`mailAprob`), liberar turno (`mailLiberacion`). Está en el **límite de 6 templates** del plan gratuito/Personal.

- **Cuenta B (legacy, NO USAR — dejar de a poco)** — `service_yeb4aqb`, public key `TzbSjqDNPjTGSGdzN`, conectada directo al Gmail personal `dcamargo70@gmail.com`. Hospedaba el template "Recordatorio Instructor" y el de vencimiento porque la Cuenta A no tenía cupo de templates en su momento.

> **Bug real encontrado y corregido (2026-06-28):** los dos crons server-side (§20 y §21) seguían apuntando a la Cuenta B (`service_yeb4aqb`/`TzbSjqDNPjTGSGdzN`) porque **nunca se actualizaron cuando se consolidó el front-end a la Cuenta A** — esa consolidación solo tocó las funciones que corren desde el navegador (`mailCancel`/`mailReset`/`mailNuevoRegistro`/`mailAprob`/`mailLiberacion`), no los workflows de GitHub Actions. Por eso "Turno Caído" y el "Recordatorio" al instructor seguían saliendo desde `dcamargo70@gmail.com` mucho después de que el resto ya saliera por el club. **Corregido:** los dos `.yml` ahora usan `service_8yqlptz` + key `0IoRm88tZQepkHAdK`. Pendiente de **confirmación real** (ver §18 y §20/§21) — el `dry_run` no llega a probar el envío en sí, solo la lógica.
>
> **Importante si se vuelve a tocar esto:** las **plantillas viven a nivel de cuenta** de EmailJS, no por servicio — si `template_8awr1zd` ("Recordatorio") o `template_4wja6rq` ("Turno Caído") solo existen en la Cuenta B, hay que **recrearlas en la Cuenta A** (no se "mueven" solas), igual que se hizo en su momento con registro/bienvenida. El secreto `EMAILJS_PRIVATE_KEY` en GitHub también es por cuenta — si apuntaba a la Cuenta B, hay que reemplazarlo por la private key de la Cuenta A (Account → API Keys, logueado con la cuenta que tiene `service_8yqlptz`).
...
### EmailJS (Cuenta A, club — ver §13. Antes Cuenta B/dcamargo70, corregido 2026-06-28)
- Service `service_8yqlptz`, template `template_8awr1zd` ("Recordatorio Instructor"), public key `0IoRm88tZQepkHAdK`.
...
- El mail sale **desde `aeroclubgra@gmail.com`** (Gmail conectado a `service_8yqlptz`). Reply-To = mail del club (`CLUB_EMAIL`). **Pendiente de confirmación real** tras el cambio de cuenta (ver §18) — falta que se dispare un caso real (turno aprobado dentro de la ventana de `REMIND_HOURS`) para confirmar que el envío entra y sale por `aeroclubgra` y no por `dcamargo70`.

### Configuración (variables `env:` del workflow)
- Públicas (en el `.yml`): `FIREBASE_DB_URL`, `EMAILJS_SERVICE_ID` (`service_8yqlptz`), `EMAILJS_TEMPLATE_ID` (`template_8awr1zd`), `EMAILJS_PUBLIC_KEY` (`0IoRm88tZQepkHAdK`), `CLUB_EMAIL` (`administracion@aeroclubriogrande.com`), `REMIND_HOURS` (**12**), `TZ_OFFSET` (`-3`), `DRY_RUN`.
- **Secreta** (GitHub → Settings → Secrets and variables → Actions): `EMAILJS_PRIVATE_KEY` = private key de la **Cuenta A** (logueada con `dcamargo70` pero la key es la de la cuenta que tiene `service_8yqlptz`/`0IoRm88tZQepkHAdK`). **Nunca** va en el repo.
...
### Configuración (variables `env:` del workflow)
- Públicas: `FIREBASE_DB_URL`, `EMAILJS_SERVICE_ID` (`service_8yqlptz`, Cuenta A/club — antes `service_yeb4aqb`, corregido 2026-06-28), `EMAILJS_TEMPLATE_ID` (`template_4wja6rq`, template del aviso de vencimiento al alumno, ahora en Cuenta A), `EMAILJS_PUBLIC_KEY` (`0IoRm88tZQepkHAdK`), `CLUB_EMAIL` (Reply-To), `EXPIRE_HOURS` (default **6**, debe ser <12), `TZ_OFFSET` (`-3`), `FPL_PURGE_HOURS` (default **1**), `DRY_RUN`.
- **Secreta:** `EMAILJS_PRIVATE_KEY` (accessToken de la Cuenta A, en GitHub Secrets). Mismo secreto que usa §20 (un solo secret para los dos workflows).
